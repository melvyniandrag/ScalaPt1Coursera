...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def updateAcc(char: Char, accList: List[(Char, Int)]): List[(Char, Int)] = accList match {
        case Nil => List((char, 1))
        case y::ys => if (char == y._1) (char, y._2 + 1) :: ys else y :: updateAcc(char, ys) 
      }
      def timesAcc(charList: List[Char], accList: List[(Char, Int)]): List[(Char, Int)] = charList match {
        case Nil => accList
        case y::ys => timesAcc(ys, updateAcc(y, accList))
      }
      timesAcc(chars, Nil)
    }
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
      def updateAccList(pair: (Char, Int), accList: List[Leaf]): List[Leaf] = accList match{
        case Nil => List(Leaf(pair._1, pair._2))
        case y :: ys => if  (pair._2 <= weight(y)) Leaf(pair._1, pair._2) :: accList else y :: updateAccList(pair, ys)
      }
      def orderedAcc(freqList: List[(Char, Int)], accList: List[Leaf]): List[Leaf] = freqList match{
        case Nil => accList
        case  y :: ys => orderedAcc(ys, updateAccList(y, accList))
      }
      orderedAcc(freqs, Nil)
    }
   ...
   */
    ...
    def insert(tree: CodeTree, mergedList: List[CodeTree]): List[CodeTree] = mergedList match {
      case Nil => List(tree)
      case x :: xs => if (weight(tree) <= weight(x)) tree :: mergedList else x :: insert(tree, xs)
    }
    
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      case x :: y :: z => insert(Fork(x, y, chars(x):::chars(y), weight(x)+weight(y)), z)
      case x => trees
    }
   ...
   */
    ...
    def until(singletonFun: List[CodeTree] => Boolean , combineFun: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): List[CodeTree] = {
      if (trees == Nil) {
        trees
      } else if (singletonFun(trees)){
        trees
      } else {
        until(singletonFun, combineFun)(combine(trees))
      }
    }
   ...
   def decodeAcc(tree: CodeTree, subTree: CodeTree, bitList: List[Bit], accList: List[Char]): List[Char] = subTree match {
     ...
     case f: Fork => if (bitList.head == 0) {
         decodeAcc(tree, left(subTree), bitList.tail, accList)
           ...
           decodeAcc(tree, right(subTree), bitList.tail, accList)     
         } else{
           throw new IllegalArgumentException("non-binary bit passed in bit list.")
         }
   }
   
   def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
     decodeAcc(tree, tree, bits, Nil)
   }
   ...
   */
   def codeBits(table: CodeTable)(char: Char): List[Bit] = table match{
      case Nil => throw new IllegalArgumentException("empty table passed to codeBits.")
   ...
   */
    ...
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      ...
      quickEncoderAcc(codeTable, text, Nil)
    }
  }
